...
def convert_language_to_sql(prompt):
    ...
    instruction = '''Your job is to convert a natural language question to an SQLite query. Use the following database schema to construct the queries. When building the query, always use "SELECT *" so that as much information is received as possible. Always JOIN PlayerStats and GameInfo to get the most information as possible. JOIN multiple tables together if necessary. Provide no other output other than the final SQL query.
            SCHEMA:
...
def results_cleaner(question, answer):
    ...
    instruction = '''You will receive a prompt in the format:
        ...
        Provide a concise response that communicates the answer to the user's question. Do NOT respond with JSON data. You respond with poor grammar and no capitalization or punctuation.
    '''
